...
{
    protected string $comboName = 'четверка';
    ...
    public function comboExist()
    {
        ...
        $cardsPool->push(new Card(5,2,7,'n','s'));

        $checker = new CombosChecker($cardsPool);

        echo 'Есть '.$this->comboName.': '.(int)!!$checker->check('fourOfKind').PHP_EOL;
    }
    ...
    public function comboNotExist()
    {
        ...
        echo 'Нет ' . $this->comboName . ': ' . (int)$checker->check('fourOfKind');
    }
}
