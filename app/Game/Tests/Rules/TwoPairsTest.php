...
class TwoPairsTest
{
    protected string $comboName = 'две пары';
    ...
    public function comboExist()
    {
        ...
        $cardsPool->push(new Card(15,2,2,'5','s'));
        $cardsPool->push(new Card(6,0,3,'n','s'));
        $cardsPool->push(new Card(4,2,4,'9','s'));
        $cardsPool->push(new Card(5,2,5,'n','s'));
        $cardsPool->push(new Card(2,3,6,'t','s'));

        $checker = new CombosChecker($cardsPool);

        echo 'Есть '.$this->comboName.': '.(int)!!$checker->check('twoPairs').PHP_EOL;
    }
    ...
    public function comboNotExist()
    {
        ...
        echo 'Нет ' . $this->comboName . ': ' . (int)$checker->check('twoPairs');
    }
}
